...
const LOCOMO_MAX_CASES_PER_SAMPLE = Number(
  process.env.PUBLIC_MEMORY_EVAL_LOCOMO_CASES_PER_SAMPLE ??
    Math.max(2, Math.ceil(LOCOMO_CASE_LIMIT / 5)),
);
const LONGMEMEVAL_CASE_LIMIT = Number(process.env.PUBLIC_MEMORY_EVAL_LONGMEMEVAL_CASES ?? 4);
const LONGMEMEVAL_DISTRACTORS_PER_QUERY = Number(
  process.env.PUBLIC_MEMORY_EVAL_LONGMEMEVAL_DISTRACTORS ?? 24,
);
...
const RUN_ANSWER_JUDGE =
  process.env.PUBLIC_MEMORY_EVAL_ANSWER_JUDGE === '1' || process.argv.includes('--answer-judge');
const RUN_QUERY_EXPANSION =
  process.env.PUBLIC_MEMORY_EVAL_QUERY_EXPANSION === '1' ||
  process.argv.includes('--query-expansion');
...
async function fetchJson<T>(url: string, outputPath?: string): Promise<T> {
  if (outputPath && fs.existsSync(outputPath)) {
    return readJsonFile<T>(outputPath);
  }
  ...
  return parseJsonOrThrow<T>(text, url);
}

function parseJsonOrThrow<T>(text: string, source: string): T {
  try {
...
/** Read + parse a local JSON file with a descriptive error on corruption. */
function readJsonFile<T>(filePath: string): T {
  ...
  try {
    ...
    throw new Error(
      `Corrupt JSON file ${filePath}: ${err instanceof Error ? err.message : String(err)} — delete it and re-run to re-fetch.`,
    );
  }
}
...
function locomoCategory(category: number): string {
  switch (category) {
    case 1:
      return 'locomo_single_hop';
    case 2:
      return 'locomo_temporal';
    case 3:
      return 'locomo_multi_hop';
    case 4:
      return 'locomo_open_domain';
    case 5:
      return 'locomo_adversarial';
    default:
      return `locomo_category_${category}`;
  }
}
...
async function buildLoCoMoFixture(): Promise<BuiltFixture> {
  ...
  const samples = readJsonFile<LoCoMoSample[]>(rawPath);
  ...
  for (const sample of samples) {
    ...
    for (const [key, value] of Object.entries(conversation)) {
      ...
      for (let turnIndex = 0; turnIndex < turns.length; turnIndex++) {
        ...
        const windowTurnIds = windowTurns.map(
          (item) => `locomo:${sample.sample_id}:${item.dia_id}`,
        );
    ...
    for (const [index, qa] of sample.qa.entries()) {
      ...
      if (
        (categoryCounts.get(category) ?? 0) >= categoryTarget &&
        cases.length < LOCOMO_CASE_LIMIT - 2
      ) {
        continue;
      }
      cases.push({
...
async function fetchHfRows<T>(
  ...
  const response = await fetchJson<HfRowsResponse<T>>(
    hfUrl(
      offset === 0 ? HF_FIRST_ROWS_URL : HF_ROWS_URL,
      config,
...
async function fetchCorpusRowsByIds(ids: string[]): Promise<Map<string, LongMemEvalCorpusRow>> {
  ...
  const windowOffset = Math.max(
    0,
    estimatedScene126Start + (minScene - 126) * estimatedRowsPerScene - 50,
  );
  ...
  if (fs.existsSync(rawRowsPath)) {
    rawRows = readJsonFile<Array<{ row_idx: number; row: LongMemEvalCorpusRow }>>(rawRowsPath);
  } else {
    rawRows = [];
    for (let offset = windowOffset; offset < windowOffset + windowLength; offset += 100) {
      ...
      rawRows.push(
        ...(await fetchHfRows<LongMemEvalCorpusRow>('multi_session-corpus', length, offset)),
      );
      await sleep(500);
    }
  ...
  for (const id of ids) {
    ...
    if (!found)
      throw new Error(`Could not fetch LongMemEval corpus row ${id} in selected corpus window`);
...
async function buildLongMemEvalFixture(): Promise<BuiltFixture> {
  ...
  const topRankedByQuery = new Map(
    topRankedRows.map(({ row }) => [row['query-id'], row['corpus-ids']]),
  );
  const selectedQueries = queryRows
    .filter(({ row }) => qrelsByQuery.has(row.id))
    .slice(0, LONGMEMEVAL_CASE_LIMIT);
  ...
  for (const { row } of selectedQueries) {
    ...
    for (const id of (topRankedByQuery.get(row.id) ?? []).slice(
      0,
      LONGMEMEVAL_DISTRACTORS_PER_QUERY,
    )) {
      corpusIds.add(id);
    }
  }
...
async function runEval(fixture: BuiltFixture, options: EvalRunOptions): Promise<EvalRunResult> {
  ...
  await warmEmbeddingCache(embeddingClient, embeddingCacheRepo, [
    ...fixture.memories.map((memory) => memory.content),
    ...fixture.cases.map((evalCase) => evalCase.query),
    ...fixture.cases.flatMap((evalCase) =>
      planMemoryQueries(evalCase.query).map((query) => query.query),
    ),
  ...
  const retriever = new MemoryRetriever({
    ...
    embeddingCacheRepo,
    db,
    expansionConfig: options.expansionConfig ?? {
      enabled: false,
      minQueryLength: 15,
      minScoreTrigger: 0.3,
      maxVariants: 0,
    ...
    decayConfig: { halfLifeDays: 36500 },
...
function aggregateByDataset(results: CaseResult[]): Record<string, AggregateMetrics> {
  ...
  return Object.fromEntries(
    [...grouped.entries()].map(([dataset, values]) => [dataset, aggregate(values)]),
  );
}
...
function aggregateByCategory(results: CaseResult[]): Record<string, AggregateMetrics> {
  ...
  return Object.fromEntries(
    [...grouped.entries()].map(([category, values]) => [category, aggregate(values)]),
  );
}
...
function answerAccuracyByCategory(results: CaseResult[]): Record<string, AnswerAccuracy> {
  ...
  return Object.fromEntries(
    [...grouped.entries()].map(([category, values]) => [category, aggregateAnswerAccuracy(values)]),
  );
}
...
function createConfiguredEmbeddingClient(): EmbeddingClient | undefined {
  ...
  const baseUrl =
    process.env.PUBLIC_MEMORY_EVAL_EMBEDDING_BASE_URL || process.env.EMBEDDING_BASE_URL;
  const model = process.env.PUBLIC_MEMORY_EVAL_EMBEDDING_MODEL || process.env.EMBEDDING_MODEL;
  const dimension = Number(
    process.env.PUBLIC_MEMORY_EVAL_EMBEDDING_DIMENSION || process.env.EMBEDDING_DIMENSION || 0,
  );
...
function createConfiguredJudge(): JudgeConfig | undefined {
  ...
  if (!modelRef || !apiKey || !baseUrl) {
    console.warn(
      '[answer-judge] requested but PUBLIC_MEMORY_EVAL_JUDGE_{MODEL,API_KEY,BASE_URL} not all set — skipping',
    );
    return undefined;
  }
 ...
 */
function createConfiguredExpansion(overrides?: {
  minQueryLength?: number;
}): LLMExpansionConfig | undefined {
  if (!RUN_QUERY_EXPANSION) return undefined;
  const modelRef =
    process.env.PUBLIC_MEMORY_EVAL_EXPANSION_MODEL || 'nvidia/deepseek-ai/deepseek-v4-flash';
  const apiKey = process.env.PUBLIC_MEMORY_EVAL_EXPANSION_API_KEY || process.env.PI_AI_API_KEY;
  const baseUrl =
    process.env.PUBLIC_MEMORY_EVAL_EXPANSION_BASE_URL || 'https://integrate.api.nvidia.com/v1';
  if (!apiKey) {
    console.warn(
      '[query-expansion] requested but no API key (set PUBLIC_MEMORY_EVAL_EXPANSION_API_KEY or PI_AI_API_KEY) — skipping',
    );
    return undefined;
  }
  const provider = modelRef.includes('/') ? modelRef.split('/')[0] : '*';
...
async function warmEmbeddingCache(
  ...
  const missing = uniqueTexts.filter(
    (text) => !embeddingCacheRepo.get(hashContent(text, embeddingClient.model)),
  );
  ...
  try {
    const embeddings = await withTimeout(
      embeddingClient.embed
        ...
        : Promise.all(batch.map((text) => embeddingClient.embedOne(text))),
    ...
    if (batch.length === 1) {
      console.warn(
        `Embedding cache warmup skipped one item: ${err instanceof Error ? err.message : String(err)}`,
      );
      return;
    }
...
function metricsTable(
  metricsByDataset: Record<string, AggregateMetrics>,
...
function laneCategoryTable(runs: EvalRunResult[]): string {
  ...
  const categories = [
    ...new Set(active.flatMap((run) => run.results.map((r) => r.category))),
  ].sort();
  ...
  const rows = active.map((run) => {
    ...
    return [
      run.lane,
      ...categories.map((cat) => (byCat[cat] ? formatPercent(byCat[cat].hitAt10) : '-')),
    ];
  });
  return [
    `| ${header.join(' | ')} |`,
...
function answerAccuracyTable(
  byCategory: Record<string, AnswerAccuracy>,
  overall: AnswerAccuracy,
): string {
...
function answerReportLines(results: CaseResult[]): string[] {
  ...
  return ['', answerAccuracyTable(answerAccuracyByCategory(results), acc)];
}
...
function writeReport(
  ...
  const v11Report = computeV11EvalReport(
    results.map((result) => ({
      id: result.id,
...
function assertGate(run: EvalRunResult): void {
  ...
  if (!locomo || locomo.hitAt10 < 0.6)
    failures.push(`LoCoMo Hit@10 ${formatPercent(locomo?.hitAt10 ?? 0)} < 60.0%`);
  if (!longMemEval || longMemEval.hitAt10 < 0.9)
    failures.push(`LongMemEval Hit@10 ${formatPercent(longMemEval?.hitAt10 ?? 0)} < 90.0%`);
  if (overall.hitAt10 < 0.7)
    failures.push(`overall Hit@10 ${formatPercent(overall.hitAt10)} < 70.0%`);
  if (run.metrics.p95LatencyMs > 2_000)
    failures.push(`p95 ${formatMs(run.metrics.p95LatencyMs)} > 2000.0 ms`);
  ...
  if (configuredEmbeddingClient) {
    ...
    if (expansionConfig) {
      ...
      runs.push({
        ...
        embeddingCount: 0,
        skippedReason:
          'set --query-expansion (or PUBLIC_MEMORY_EVAL_QUERY_EXPANSION=1) + an expansion API key',
      });
    }
