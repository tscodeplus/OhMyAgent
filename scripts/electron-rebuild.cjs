...
const nativeModules = ['better-sqlite3', 'sqlite-vec', 'sharp', '@nut-tree-fork/nut-js'].join(',');
...
try {
  execSync(`npx electron-rebuild -f -w ${nativeModules}`, { cwd: desktopDir, stdio: 'inherit' });
...
if (os.platform() === 'darwin') {
  ...
  try {
    execSync('node scripts/make-universal-addons.cjs', { cwd: desktopDir, stdio: 'inherit' });
  } catch (err) {
    console.warn(
