...
const cfg = loadConfig({ OHMYAGENT_HOME: '/tmp/oma-probe-home' } as NodeJS.ProcessEnv, '/tmp/oma-max-config.yaml');
console.log('VALIDATED OK');
console.log('piAi=', JSON.stringify(cfg.piAi));
console.log('telegram.enabled=', cfg.telegram?.enabled, 'botToken len=', cfg.telegram?.botToken?.length);
