    ...
    <div className="flex h-full flex-col bg-white dark:bg-neutral-950">
        ...
        <div className="flex items-center gap-3">
          {subTabs.map((tab) => {
            ...
            return (
              ...
              >
                <Icon
                  className={`h-3.5 w-3.5 ${tab.id === 'manage' ? 'text-amber-500' : 'text-violet-500'}`}
                  strokeWidth={1.75}
                />
      ...
      {activeTab === 'manage' && (
        <div className="flex min-h-0 flex-1">
          ...
          <div
            className={`flex w-72 max-sm:w-full shrink-0 flex-col border-r max-sm:border-r-0 border-neutral-200 dark:border-neutral-800 ${mobileShowEditor ? 'max-sm:hidden' : ''}`}
          >
          ...
          <div
            className={`flex min-h-0 flex-1 flex-col ${!mobileShowEditor ? 'max-sm:hidden' : ''}`}
          >
            {activeSkill ? (
              <>
                {/* Info bar */}
              ...
              <div className="flex flex-1 items-center justify-center text-[13px] text-neutral-500 dark:text-neutral-400 max-sm:hidden">
                {skills.length > 0
                  ? t('skills.selectPrompt', { defaultValue: 'Select a skill to edit' })
                  : t('skills.empty')}
              </div>
            )}
          </div>
      ...
      {showNewModal && (
        <Modal
          ...
          title={t('skills.createSkill') as string}
          size="md"
          footer={
        ...
        >
          <div className="space-y-4">
            ...
            <Input
              ...
              placeholder={t('skills.descriptionPlaceholder') as string}
            />
          </div>
        </Modal>
      )}
