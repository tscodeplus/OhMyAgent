...
export default function AppShell() {
  ...
  useEffect(() => {
    return subscribe('config_changed', (data: any) => {
      ...
      if (
        ...
        Array.isArray(data.restartReasons) &&
        data.restartReasons.length > 0
      ) {
  ...
  const [sidebarWidth, setSidebarWidth] = useState(() => {
    try {
      const n = Number(localStorage.getItem('oma-sidebar-width'));
  ...
  const [sidebarOpen, setSidebarOpen] = useState(() => {
    try {
      return localStorage.getItem('oma-sidebar-collapsed') !== 'true';
    } catch {
      return true;
    }
  });
  ...
  const [toolsOpen, setToolsOpen] = useState(() => {
    try {
      return localStorage.getItem('oma-sidebar-tools-open') !== 'false';
    } catch {
      return true;
    }
  });
  const toggleTools = useCallback(() => {
    setToolsOpen((v) => {
      const n = !v;
      try {
        localStorage.setItem('oma-sidebar-tools-open', String(n));
      } catch {}
      return n;
    });
  }, []);
  ...
  const toggleSidebar = useCallback(() => {
    ...
    setSidebarOpen((v) => {
      const n = !v;
      try {
        localStorage.setItem('oma-sidebar-collapsed', String(!n));
      } catch {}
      return n;
    });
  ...
  const handleResizeStart = useCallback(
    (e: React.MouseEvent) => {
      ...
      const mm = (ev: MouseEvent) =>
        setSidebarWidth(Math.min(SIDEBAR_MAX, Math.max(SIDEBAR_MIN, sw + ev.clientX - sx)));
      const mu = () => {
        setIsResizing(false);
        ...
        queueMicrotask(() => {
          ...
          if (w && Number.isFinite(w))
            try {
              localStorage.setItem('oma-sidebar-width', String(Math.round(w)));
            } catch {}
        });
      };
    ...
    const load = async () => {
      try {
        ...
        setCurrentSessionTitle(
          s ? s.title || (s.metadata && (s.metadata as any).title) || t('chat.newSession') : null,
        );
      } catch {
  ...
  useEffect(() => {
    if (sidebarVisible) setChatNavOpen(false);
  }, [sidebarVisible]);
  ...
  const isTabActive = (tab: Tab) =>
    tab.path === '/'
      ? isChatArea || location.pathname === '/'
      : location.pathname.startsWith(tab.path);

  // Caption strip text (subtle, so the shell still reads as toolbar-free).
  const pageTitle = location.pathname.startsWith('/skills')
    ? t('nav.skills')
  ...
  const sidebarEl = (
    ...
    >
      ...
      >
        ...
        <div className="flex h-16 shrink-0 items-center justify-between pl-2 pr-4">
          <button
            type="button"
            onClick={() => navigate('/')}
            className="flex items-center gap-2 rounded-md p-1 transition hover:opacity-80"
          >
            <BrandMark className="h-6 w-6" />
            <span className="text-[15px] font-semibold tracking-tight text-neutral-800 dark:text-neutral-200">
              OhMyAgent
            </span>
          </button>
          <button
        ...
        <div className="mt-4 shrink-0 border-t border-neutral-200/70 dark:border-neutral-800/70">
          ...
          >
            <ChevronDown
              className={`h-3.5 w-3.5 shrink-0 transition-transform duration-200 ${toolsOpen ? '' : '-rotate-90'}`}
              strokeWidth={1.75}
            />
            <span className="truncate text-[13px]">
              {toolsOpen ? t('sidebar.collapsePanels') : t('sidebar.expandPanels')}
            </span>
          </button>
          {toolsOpen && (
      ...
      <div
        className={`absolute inset-0 flex flex-col items-center px-[10px] pb-2 pt-[18px] transition-opacity duration-200 ${sidebarVisible || mobileSidebar ? 'pointer-events-none opacity-0' : 'opacity-100'}`}
      >
      ...
      {sidebarVisible && (
        <div
          ...
          onDoubleClick={() => {
            ...
            try {
              localStorage.setItem('oma-sidebar-width', String(SIDEBAR_DEFAULT));
            } catch {}
          }}
