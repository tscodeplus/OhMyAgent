...
export default function ComputerUseSettings({
  tabId = 'computer',
  registerHandle,
  onDirtyChange,
        ...
        <div className="space-y-4 rounded-lg border border-neutral-200 dark:border-neutral-800 bg-white dark:bg-neutral-900 p-4">
          <div className="flex items-center justify-between">
            <label className="text-sm font-medium text-neutral-900 dark:text-neutral-100">
              {t('settings.computer.enabled')}
            </label>
            <Toggle
              checked={getField('computerUse.enabled', !!cu.enabled) as boolean}
              onChange={(v) => setField('computerUse.enabled', v)}
            />
          </div>
          <Select
            label="Provider"
            value={getField('computerUse.provider', String(cu.provider || 'auto')) as string}
        ...
        <div className="space-y-4 rounded-lg border border-neutral-200 dark:border-neutral-800 bg-white dark:bg-neutral-900 p-4">
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
            ...
            <Input
              label="Port"
              type="number"
              value={getField('computerUse.ssh.port', String(cuSsh.port || 22)) as string}
          ...
          <Input
            label="Jump Host"
            value={getField('computerUse.ssh.jumpHost', String(cuSsh.jumpHost || '')) as string}
