...
export default function ToolsPolicySettings({
  tabId = 'tools',
  registerHandle,
  onDirtyChange,
}: ToolsPolicySettingsProps) {
  ...
  return (
    <div className="space-y-3">
      <AccordionItem title={t('settings.policy.globalPolicy')}>
        <Select
          label={t('settings.policy.modeLabel')}
          value={getField('policy.mode', str(policy.mode, 'balanced')) as string}
          onChange={(e) => setField('policy.mode', e.target.value)}
          options={[
            { value: 'bypass', label: t('settings.policy.opt_bypass') },
            { value: 'permissive', label: t('settings.policy.opt_permissive') },
            { value: 'balanced', label: t('settings.policy.opt_balanced') },
            { value: 'safe', label: t('settings.policy.opt_safe') },
          ]}
        />
        <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
          <Input
            label={t('settings.policy.approvalTimeout')}
            type="number"
            value={getField('policy.approval.timeoutSec', str(approval.timeoutSec, '')) as string}
            onChange={(e) => setField('policy.approval.timeoutSec', e.target.value)}
          />
          <Select
            label={t('settings.policy.timeoutAction')}
            value={
              getField(
                'policy.approval.timeoutAction',
                str(approval.timeoutAction, 'deny'),
              ) as string
            }
            onChange={(e) => setField('policy.approval.timeoutAction', e.target.value)}
            options={[
              { value: 'allow', label: t('settings.policy.opt_allow') },
              { value: 'deny', label: t('settings.policy.opt_deny') },
            ]}
          />
        </div>
      </AccordionItem>
      ...
      <AccordionItem title={t('settings.tools.shell')}>
        <div className="flex items-center justify-between">
          <label className="text-sm font-medium text-neutral-900 dark:text-neutral-100">
            {t('settings.tools.shellEnabled')}
          </label>
          <Toggle
            checked={getField('tools.shellEnabled', !!tools.shellEnabled) as boolean}
            onChange={(v) => setField('tools.shellEnabled', v)}
          />
        </div>
        <Select
          label={t('settings.tools.execMode')}
          value={getField('tools.shellExecMode', str(tools.shellExecMode, 'balanced')) as string}
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
          <Input
            label={t('settings.tools.defaultTimeout')}
            type="number"
            value={getField('tools.defaultTimeoutMs', str(tools.defaultTimeoutMs, '')) as string}
            onChange={(e) => setField('tools.defaultTimeoutMs', e.target.value)}
          />
          <Input
            label={t('settings.tools.maxOutput')}
            type="number"
            value={getField('tools.maxOutputLength', str(tools.maxOutputLength, '')) as string}
          ...
          <Input
            ...
            value={
              getField(
                ...
                str(tools.shellApprovalTimeoutSec, ''),
              ) as string
            }
            onChange={(e) => setField('tools.shellApprovalTimeoutSec', e.target.value)}
          />
          <Select
            label={t('settings.tools.timeoutAction')}
            value={
              getField(
                'tools.shellApprovalTimeoutAction',
                str(tools.shellApprovalTimeoutAction, 'deny'),
              ) as string
            }
            onChange={(e) => setField('tools.shellApprovalTimeoutAction', e.target.value)}
            options={[
              { value: 'deny', label: t('settings.tools.opt_deny') },
              { value: 'allow', label: t('settings.tools.opt_allow') },
            ]}
          />
        </div>
        <Select
          label={t('settings.tools.approvalMode')}
          value={
            getField('tools.shellApprovalMode', str(tools.shellApprovalMode, 'balanced')) as string
          }
          onChange={(e) => setField('tools.shellApprovalMode', e.target.value)}
          options={[
            { value: 'balanced', label: t('settings.tools.opt_balanced') },
            { value: 'relaxed', label: t('settings.tools.opt_relaxed') },
            { value: 'strict', label: t('settings.tools.opt_strict') },
          ]}
        />
      </AccordionItem>
      ...
      <AccordionItem title={t('settings.tools.fileRead')}>
        <Input
          label={t('settings.tools.allowedRoots')}
          value={
            getField(
              'tools.fileRead.allowedRoots',
              Array.isArray(fileRead.allowedRoots)
                ? (fileRead.allowedRoots as string[]).join(', ')
                : '',
            ) as string
          }
        ...
        />
        <Input
          label={t('settings.tools.deniedPatterns')}
          value={
            getField(
              'tools.fileRead.deniedPatterns',
              Array.isArray(fileRead.deniedPatterns)
                ? (fileRead.deniedPatterns as string[]).join(', ')
                : '',
            ) as string
          }
