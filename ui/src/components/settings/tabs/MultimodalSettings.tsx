...
export default function MultimodalSettings({
  tabId = 'multimodal',
  registerHandle,
  onDirtyChange,
}: MultimodalSettingsProps) {
  ...
  // Only offer providers that actually have an API key configured (same as the
  // Model/Routing and Agent editor pickers), and let the model dropdown support
  // search + custom model ids via ModelPicker.
  ...
  return (
    <div className="space-y-3">
      ...
      <div className="flex items-center justify-between rounded-lg border border-neutral-200 dark:border-neutral-800 bg-white dark:bg-neutral-900 px-4 py-3">
        ...
        <Toggle
          checked={getField('multimodal.enabled', !!mm.enabled) as boolean}
          onChange={(v) => setField('multimodal.enabled', v)}
        />
      </div>

      <AccordionItem title={t('settings.multimodal.image')}>
        <Select
          label={t('settings.multimodal.imageMode')}
          value={
            getField(
              'multimodal.image.mode',
              String((mm.image as Record<string, unknown>)?.mode || 'native_first'),
            ) as string
          }
          onChange={(e) => setField('multimodal.image.mode', e.target.value)}
          options={[
            { value: 'bridge_only', label: t('settings.multimodal.opt_bridge_only') },
            { value: 'native_first', label: t('settings.multimodal.opt_native_first') },
            { value: 'native_only', label: t('settings.multimodal.opt_native_only') },
          ]}
        />
        ...
        {(() => {
          const bri =
            ((mm.image as Record<string, unknown>)?.bridge as Record<string, unknown>) || {};
          return (
            <>
              ...
              <div className="flex items-center justify-between">
                ...
                <Toggle
                  checked={getField('multimodal.image.bridge.enabled', !!bri.enabled) as boolean}
              ...
              <ModelRefInput
                ...
                value={
                  getField('multimodal.image.bridge.modelRef', String(bri.modelRef || '')) as string
                }
      ...
      <AccordionItem title={t('settings.multimodal.imageGeneration')}>
        <div className="flex items-center justify-between">
          <label className="text-sm font-medium text-neutral-900 dark:text-neutral-100">
            {t('settings.multimodal.enabled')}
          </label>
          <Toggle
            checked={
              getField(
                'multimodal.imageGeneration.enabled',
                !!(mm.imageGeneration as Record<string, unknown>)?.enabled,
              ) as boolean
            }
        ...
        <ModelRefInput
          ...
          value={
            getField(
              'multimodal.imageGeneration.modelRef',
              String((mm.imageGeneration as Record<string, unknown>)?.modelRef || ''),
            ) as string
          }
          onChange={(v) => setField('multimodal.imageGeneration.modelRef', v)}
        />
        <Input
          label={t('settings.multimodal.outputDir')}
          value={
            getField(
              'multimodal.imageGeneration.outputDir',
              String((mm.imageGeneration as Record<string, unknown>)?.outputDir || ''),
            ) as string
          }
          onChange={(e) => setField('multimodal.imageGeneration.outputDir', e.target.value)}
        />
        <Input
          label={t('settings.multimodal.maxPromptChars')}
          type="number"
          value={
            getField(
              'multimodal.imageGeneration.maxPromptChars',
              String((mm.imageGeneration as Record<string, unknown>)?.maxPromptChars || ''),
            ) as string
          }
      ...
      <AccordionItem title={t('settings.multimodal.videoGeneration')}>
        <div className="flex items-center justify-between">
          ...
          <Toggle
            checked={
              getField(
                'multimodal.videoGeneration.enabled',
                !!(mm.videoGeneration as Record<string, unknown>)?.enabled,
              ) as boolean
            }
        ...
        <ModelRefInput
          ...
          value={
            getField(
              'multimodal.videoGeneration.modelRef',
              String((mm.videoGeneration as Record<string, unknown>)?.modelRef || ''),
            ) as string
          }
        ...
        <Input
          ...
          value={
            getField(
              'multimodal.videoGeneration.outputDir',
              String((mm.videoGeneration as Record<string, unknown>)?.outputDir || ''),
            ) as string
          }
        ...
        <Input
          ...
          value={
            getField(
              ...
              String((mm.videoGeneration as Record<string, unknown>)?.maxPromptChars || ''),
            ) as string
          }
          onChange={(e) => setField('multimodal.videoGeneration.maxPromptChars', e.target.value)}
        />
        <Input
          label={t('settings.multimodal.defaultSeconds')}
          value={
            getField(
              'multimodal.videoGeneration.defaultSeconds',
              String((mm.videoGeneration as Record<string, unknown>)?.defaultSeconds || '5.0'),
            ) as string
          }
          onChange={(e) => setField('multimodal.videoGeneration.defaultSeconds', e.target.value)}
        />
        <Input
          label={t('settings.multimodal.defaultSize')}
          value={
            getField(
              'multimodal.videoGeneration.defaultSize',
              String((mm.videoGeneration as Record<string, unknown>)?.defaultSize || '1280x768'),
            ) as string
          }
      ...
      <AccordionItem title={t('settings.multimodal.stt')}>
        <div className="flex items-center justify-between">
          ...
          <Toggle
            checked={
              getField(
                'multimodal.stt.enabled',
                !!(mm.stt as Record<string, unknown>)?.enabled,
              ) as boolean
            }
        ...
        <Input
          label={t('settings.multimodal.language')}
          value={
            getField(
              'multimodal.stt.language',
              String((mm.stt as Record<string, unknown>)?.language || 'auto'),
            ) as string
          }
          onChange={(e) => setField('multimodal.stt.language', e.target.value)}
        />
        <Input
          label={t('settings.multimodal.maxDuration')}
          type="number"
          value={
            getField(
              'multimodal.stt.maxDurationSec',
              String((mm.stt as Record<string, unknown>)?.maxDurationSec || 300),
            ) as string
          }
      ...
      <AccordionItem title={t('settings.multimodal.attachments')}>
        <Input
          label={t('settings.multimodal.cacheDir')}
          value={
            getField(
              'multimodal.attachments.cacheDir',
              String((mm.attachments as Record<string, unknown>)?.cacheDir || ''),
            ) as string
          }
        ...
        <div className="flex items-center justify-between">
          ...
          <Toggle
            checked={
              getField(
                'multimodal.attachments.autoParseImages',
                !!(mm.attachments as Record<string, unknown>)?.autoParseImages,
              ) as boolean
            }
