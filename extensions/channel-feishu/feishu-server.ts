...
import { decryptEvent, authenticateEvent, type FeishuAuthConfig } from './feishu-auth.js';
...
export interface FeishuServerOptions {
  ...
  cardActionHandler?: (action: FeishuCardActionCallback) => Promise<Record<string, unknown> | void>;
  logger?: any;
  rateLimit?: {
    maxRequests: number;
    windowMs: number;
 ...
 */
export function createFeishuServer(options: FeishuServerOptions): FastifyInstance<http.Server> {
  const { port, host = '0.0.0.0', feishuAuth, feishuRouter, cardActionHandler, logger } = options;
  ...
  app.post('/webhook/event', async (request: FastifyRequest, reply: FastifyReply) => {
    ...
    if (!rateLimiter.check(clientIp)) {
      return reply.code(429).send({ code: -1, msg: i18n.t('feishu-server:rateLimited') });
    }

    const body = request.body as Record<string, unknown>;
    const rawBody = request.rawBody;
    const signature = (body.signature as string) || (request.headers['x-lark-signature'] as string);
    const timestamp =
      (body.timestamp as string) || (request.headers['x-lark-request-timestamp'] as string);
    const nonce = (body.nonce as string) || (request.headers['x-lark-request-nonce'] as string);
    ...
    if (body.encrypt && feishuAuth.encryptKey) {
      try {
        const decrypted = decryptEvent(body.encrypt as string, feishuAuth.encryptKey);
  ...
  app.post('/webhook/card', async (request: FastifyRequest, reply: FastifyReply) => {
    // Rate limit check
    const clientIp = request.ip;
    if (!rateLimiter.check(clientIp)) {
      return reply.code(429).send({ code: -1, msg: i18n.t('feishu-server:rateLimited') });
    ...
    const rawBody = request.rawBody;
    const signature =
      ((body as any).signature as string) || (request.headers['x-lark-signature'] as string);
    const timestamp =
      ((body as any).timestamp as string) ||
      (request.headers['x-lark-request-timestamp'] as string);
    const nonce =
      ((body as any).nonce as string) || (request.headers['x-lark-request-nonce'] as string);
    ...
    if (cardActionHandler) {
      try {
        ...
        return reply.code(500).send({ code: -1, msg: i18n.t('feishu-server:cardActionError') });
      }
    }
