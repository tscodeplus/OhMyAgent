...
function isRateLimited(err: unknown): boolean {
  ...
  return (
    msg.includes(String(RATE_LIMIT_CODE)) ||
    msg.includes('200400') ||
...
export function isCardIdInvalidError(err: unknown): boolean {
  ...
  const message = String((obj as { message?: string }).message ?? msg ?? String(err)).toLowerCase();
...
async function extractBufferFromResponse(
  ...
  throw new Error(
    '[feishu-client] Unable to extract binary data from response: ' + typeof response,
  );
}
...
export class FeishuClient {
  ...
  /** Region-aware API origin, e.g. https://open.feishu.cn or https://open.larksuite.com */
  private readonly apiBase: string;

  constructor(config: FeishuConfig, logger: Logger) {
    this.appId = config.appId;
    this.appSecret = config.appSecret;
    // Region-aware API base — previously parsed from config but never used,
    // so international Lark users hit the wrong domain on every call.
    this.apiBase =
      config.region === 'lark' ? 'https://open.larksuite.com' : 'https://open.feishu.cn';
  ...
  private async fetchApi(
    ...
    if (!response.ok) {
      const text = await response.text();
      // Token expired between refresh and use (race window): invalidate and
      // retry once with a fresh token instead of surfacing the error upstream.
      if (
        _retryOnAuthFailure &&
        (response.status === 400 || response.status === 401) &&
        /invalid\s*access\s*token|token.*expired|9999166[138]/i.test(text)
      ) {
        this.invalidateToken();
        return this.fetchApi(method, path, body, false);
      }
  ...
  async sendMessage(params: FeishuSendMessageParams): Promise<FeishuApiResponse<SendMessageData>> {
    this.logger.debug(
      { receiveId: params.receive_id, msgType: params.msg_type },
  ...
  async replyMessage(
    ...
    const result = await this.fetchApi('POST', `/im/v1/messages/${messageId}/reply`, body);
  ...
  async updateMessage(
    ...
    const result = await this.fetchApi('PATCH', `/im/v1/messages/${messageId}`, {
      msg_type: msgType,
  ...
  async recallMessage(messageId: string): Promise<void> {
    ...
    const result = await this.fetchApi('DELETE', `/im/v1/messages/${messageId}`);
  ...
  async updateCard(
    ...
    await this.withRetry(async () => {
      this.logger.debug(
        { cardId, sequence, cardDataKeys: Object.keys(cardData) },
        'CardKit card.update called',
      );
  ...
  async setCardStreamingMode(
    ...
    await this.withRetry(async () => {
      ...
      if (resp.code && resp.code !== 0) {
        ...
        this.logger.debug(
          { code: resp.code, msg: resp.msg, cardId, streamingMode, sequence },
          err.message,
        );
        throw err;
      }
      this.logger.debug(
        { cardId, streamingMode, sequence },
        'CardKit setCardStreamingMode success',
      );
    }, 'setStreamingMode');
  }
  ...
  async sendCardByCardId(
    ...
    for (let attempt = 0; attempt <= CARD_ID_RETRY_ATTEMPTS; attempt++) {
      try {
        ...
        if (!response?.code || response.code === 0) {
          ...
          if (messageId) {
            this.logger.debug(
              { chatId, cardId, messageId, replyToMessageId },
              'CardKit card message sent',
            );
            return messageId;
          }
        ...
        //   err.response.data.code / …     — nested axios
        const code =
          err?.code ?? err?.data?.code ?? err?.response?.code ?? err?.response?.data?.code;
        const msg =
          err?.msg ??
          err?.data?.msg ??
          err?.response?.msg ??
    ...
    throw new Error(
      `sendCardByCardId error ${lastError?.code ?? 'unknown'}: ${lastError?.msg ?? 'no response'}`,
    );
  }
  ...
  async setTyping(chatId: string): Promise<void> {
    try {
      await this.fetchApi('POST', `/im/v1/chats/${chatId}/user_typing_status`, {
        action: 'typing',
      });
    } catch (error) {
  ...
  async addReaction(messageId: string, emojiType: string = 'Typing'): Promise<string | null> {
    try {
      const result = await this.fetchApi('POST', `/im/v1/messages/${messageId}/reactions`, {
  ...
  async uploadImage(
    ...
    if (!imageKey) {
      throw new Error(
        `Image upload failed: no image_key in response. Response: ${JSON.stringify(response).slice(0, 200)}`,
      );
    }
  ...
  async uploadFile(
    ...
    if (!fileKey) {
      throw new Error(
        `File upload failed: no file_key in response for "${fileName}". Response: ${JSON.stringify(response).slice(0, 200)}`,
      );
    }
