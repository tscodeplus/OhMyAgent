    ...
    it('detects identical_retry_loop (4x same failed adb connect)', () => {
      ...
      const ctx: FailureContext = {
        ...
        toolCalls: [
          ...
          {
            name: 'shell',
            args: { cmd: 'adb connect 192.168.1.100' },
            isError: true,
      ...
      const mockLLM = async (_sys: string, _msg: string): Promise<string> => {
        ...
        if (callCount === 1) {
          return JSON.stringify({
            terminal_cause: 'identical_retry_loop',
            ...
            recommended_surface: 'skill:android-operator:prompt',
            confidence: 0.88,
          });
        }
        return JSON.stringify({
