...
describe('v4 final tool surface', () => {
  ...
  it('all 37 tools have ToolDefinitions registered with proper capability descriptors', () => {
    ...
    for (const name of V4_FINAL_TOOLS) {
      ...
      if (canonical) {
        if (def.capability.category !== canonical.category) {
          mismatchedCaps.push(
            `${name}: category "${def.capability.category}" !== "${canonical.category}"`,
          );
        }
        if (def.capability.pathAccess !== canonical.pathAccess) {
          mismatchedCaps.push(
            `${name}: pathAccess "${def.capability.pathAccess}" !== "${canonical.pathAccess}"`,
          );
        }
        if (def.capability.approvalDefault !== canonical.approvalDefault) {
          mismatchedCaps.push(
            `${name}: approvalDefault "${def.capability.approvalDefault}" !== "${canonical.approvalDefault}"`,
          );
        }
      }
    }
    ...
    if (missing.length > 0) {
      issues.push(
        `Missing from registry (${missing.length}/${V4_FINAL_TOOLS.length}): ${missing.join(', ')}`,
      );
    }
