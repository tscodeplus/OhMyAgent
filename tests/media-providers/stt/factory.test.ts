...
} from '../../../src/media-providers/stt/factory.js';
...
describe('createSTTProviders', () => {
  ...
  it('creates OpenAIWhisperProvider from openai-whisper config', () => {
    const providers = createSTTProviders([{ id: 'openai-whisper', apiKey: 'sk-test' }]);
    expect(providers).toHaveLength(1);
    expect(providers[0].id).toBe('openai-whisper');
  });

  it('skips generic provider without endpoint', () => {
    const providers = createSTTProviders([{ id: 'generic', requestType: 'multipart' }]);
    expect(providers).toHaveLength(0);
  });

  it('skips openai-whisper without apiKey', () => {
    const providers = createSTTProviders([{ id: 'openai-whisper' }]);
    expect(providers).toHaveLength(0);
  });

  it('skips unknown provider id', () => {
    const providers = createSTTProviders([{ id: 'unknown-provider', apiKey: 'test' }]);
    expect(providers).toHaveLength(0);
  });
...
describe('transcribeWithFallback', () => {
  it('returns result from the first working provider', async () => {
    const providers: STTProvider[] = [new MockWorkingProvider()];
  ...
  it('falls back to second provider when first fails', async () => {
    const providers: STTProvider[] = [new MockFailingProvider(), new MockWorkingProvider()];
    const result = await transcribeWithFallback(providers, { audioPath: '/tmp/test.ogg' });
    expect(result.text).toBe('hello world');
    expect(result.providerId).toBe('mock-working');
  });
  ...
  it('throws when all providers fail', async () => {
    const providers: STTProvider[] = [new MockFailingProvider(), new MockFailingProvider()];
    await expect(transcribeWithFallback(providers, { audioPath: '/tmp/test.ogg' })).rejects.toThrow(
      'All providers failed',
    );
  });

  it('throws when no provider is available', async () => {
    const providers: STTProvider[] = [new MockUnavailableProvider()];
    await expect(transcribeWithFallback(providers, { audioPath: '/tmp/test.ogg' })).rejects.toThrow(
