...
describe('PromptManager', () => {
  ...
  describe('assemble()', () => {
    ...
    it('includeCatalogs: false omits both skills and tools catalogs', () => {
      const options = {
        availableSkills: [
          { id: 'researcher', name: 'R', description: 'D', path: 'skills/researcher/SKILL.md' },
        ],
        availableTools: [{ name: 'shell', snippet: 'Run commands' }],
        includeCatalogs: false,
      };
      const result = pm.assemble(options);
      expect(result.layers.find((l) => l.name === 'skills-catalog')).toBeUndefined();
      expect(result.layers.find((l) => l.name === 'tools-catalog')).toBeUndefined();

      // Default (undefined) still includes both
      const withCatalogs = pm.assemble({ ...options, includeCatalogs: undefined });
      expect(withCatalogs.layers.find((l) => l.name === 'skills-catalog')).toBeDefined();
      expect(withCatalogs.layers.find((l) => l.name === 'tools-catalog')).toBeDefined();
    });
