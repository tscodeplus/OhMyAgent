...
function doneEvent(): AssistantMessageEvent {
  return {
    type: 'done',
    reason: 'stop',
    message: makeMessage({ content: [{ type: 'text', text: 'ok' }] }),
  };
}
...
describe('createRetryingStreamFn', () => {
  ...
  it('retries a transient mid-stream error and succeeds on the second attempt', async () => {
    const base = vi
      .fn()
      .mockReturnValueOnce(
        makeStream([startEvent(), deltaEvent('partial '), errorEvent('fetch failed')]),
      )
  ...
  it('does not retry non-transient errors (quota/billing)', async () => {
    const base = vi
      .fn()
      .mockReturnValue(makeStream([startEvent(), errorEvent('insufficient_quota')]));
    const wrapped = createRetryingStreamFn(base, { maxRetries: 3, baseDelayMs: 1 })(
      model,
      context,
      {},
    );
  ...
  it('does not retry aborted streams', async () => {
    const base = vi
      .fn()
      .mockReturnValue(makeStream([startEvent(), errorEvent('Request was aborted')]));
    const wrapped = createRetryingStreamFn(base, { maxRetries: 2, baseDelayMs: 1 })(
      model,
      context,
      {},
