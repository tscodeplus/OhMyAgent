...
export function createAgentServices(input: AgentServicesInput): AgentServicesResult {
  ...
  const cronServiceRef: { current: import('../../cron/service.js').CronService | undefined } = {
    current: undefined,
  };
    ...
    },
    {
      ...
      onApprovalAutoReject: (requestId, reason) => {
        ...
        const resultCard = renderApprovalResultCard(
          ...
          reason === 'timeout' || reason === 'expired_before_recovery'
            ? 'timeout'
            : reason === 'steered'
              ? 'steered'
              : 'restart',
        ...
        feishuClient
          .updateMessage(req.card_message_id, 'interactive', resultCard)
          .catch((err: unknown) => {
            logger.warn({ err, requestId }, 'Failed to update approval result card — non-critical');
          });
      },
  ...
  const sendApprovalToChat = async (chatId: string, approval: any): Promise<string> => {
    if (!feishuClient?.sendApprovalCard) {
      logger.warn(
        { approvalId: approval.id, chatId },
        'No Feishu approval sender available for routed child approval',
      );
  ...
  const modelName = formatDisplayModel(resolvedDefault?.provider, resolvedDefault?.id);
  ...
  const visionBridgeConfig: VB = {
    ...
    maxCacheEntries:
      imgBridgeCfg.maxCacheEntries ?? (config as any).visionBridge?.maxCacheEntries ?? 256,
  };
  ...
  const getVisionBridge = (): VisionBridgeService | undefined => {
    ...
    if (!_visionBridge) {
      _visionBridge = new VisionBridgeService(
        visionBridgeConfig,
  ...
  const agentService = new AgentService(
    ...
    (chatId: string, messageId?: string, agentId?: string) => {
      ...
      return new ReplyDispatcher({
        ...
        showSkillCalls: config.showSkillCalls,
        logger,
      });
    },
    persistenceOpts,
