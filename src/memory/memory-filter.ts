...
export function detectCategory(text: string): MemoryCategory {
  ...
  if (
    /称呼|昵称|称谓|叫我|喊我|叫我|叫我|称我|叫我|喊我|名字.*(?:叫|是)/i.test(text) ||
    /(?:你)?(?:以后|今后|将来|往后|接下来|下次)?(?:要)?(?:称呼|叫|喊|称)(?:我|用户|其)?(?:为|作|成)?\S/i.test(
      text,
    ) ||
  ...
  // 偏好/喜欢/习惯
  if (
    /喜欢|偏好|习惯|讨厌|想要|希望.*(?:称呼|回复|回答|方式|风格)/i.test(text) ||
  ...
  // 语言/沟通风格
  if (
    /(?:用|使用|说|讲|语言).*(?:中文|英文|日文|回复|回答)/i.test(text) ||
  ...
  if (
    /需要|必须|应该|待办|todo|task|need\s+to|must|deploy|build|create|fix|update|完成|提交|部署|开发|修改|更新|修复/i.test(
      text,
    )
  ) {
    return 'task';
